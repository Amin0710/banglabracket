	...
	async function share() {
		...
		try {
			...
			if (navAny.share && navAny.canShare?.({ files: [file] })) {
				await navAny.share({
					files: [file],
					title: "My BanglaBracket",
					text: "My World Cup 2026 bracket",
				});
			} else {
				const url = URL.createObjectURL(blob);
				const a = document.createElement("a");
				a.href = url;
				a.download = "banglabracket-bracket.png";
			...
			<div
				...
				<div
					style={{
						...
						padding: "18px 26px",
						borderRadius: 18,
						background: `linear-gradient(150deg,#ffd76a,#f0a921)`,
						color: goldText,
				...
				{semis.length > 0 && (
					<div>
						...
						<div
							...
							{semis.map((s) => (
								<span key={s} className="pill" style={{ gap: 6, fontSize: 11 }}>
									<Flag name={s} size={16} />
									{s}
								</span>
							))}
...
async function drawShareImage({
	...
	const drawFlag = (name: string | null, cx: number, top: number) => {
		...
		if (img) g.drawImage(img, cx - flagW / 2, top, flagW, flagH);
			...
			g.fillStyle = "#2a3a2f";
			g.fillRect(cx - flagW / 2, top, flagW, flagH);
		}
		g.strokeStyle = "rgba(255,255,255,.25)";
		g.lineWidth = 2;
		g.strokeRect(cx - flagW / 2, top, flagW, flagH);
	};

	center(
		"BanglaBracket",
		150,
		'800 64px "Baloo Da 2", system-ui, sans-serif',
		"#f3f1e6",
	);
	center(
		"WORLD CUP 2026 · MY BRACKET",
	...
	gp.addColorStop(0, "#ffd76a");
	gp.addColorStop(1, "#f0a921");
	g.fillStyle = gp;
	roundRect(g, 120, plateY, W - 240, plateH, 32);
	g.fill();
	center(
		"MY CHAMPION",
		plateY + 66,
	...
	center(
		champion || "—",
		plateY + 210,
		'800 84px "Baloo Da 2", system-ui, sans-serif',
		goldText,
	);
	if (runnerUp)
		center(
	...
	if (semis.length) {
		...
		for (let i = 0; i < n; i++) {
			...
			g.textAlign = "center";
			g.fillText(
				semis[i].length > 12 ? semis[i].slice(0, 11) + "…" : semis[i],
				cx,
				902,
			);
		}
