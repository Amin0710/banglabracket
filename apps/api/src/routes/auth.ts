...
authRouter.post(
	...
	async (req: AuthedRequest, res) => {
		const { name, email, overseas, location } = req.body as z.infer<
			typeof signupSchema
		>;
		...
		const clash = await User.findOne({
			$or: [{ phone: { $in: candidates } }, { bkash: { $in: candidates } }],
		}).lean();
		...
		if (!bkash && !overseas)
			return res.status(400).json({ error: "bkash_or_overseas_required" });
...
authRouter.post(
	...
	limiter,
	validate(
		z.object({ email: z.string().email(), code: z.string().min(4).max(8) }),
	),
	async (req: AuthedRequest, res) => {
...
if (env.google.enabled) {
	...
	authRouter.get(
		...
		passport.authenticate("google", {
			session: false,
			failureRedirect: `${env.webUrl}${env.webAppPath}/?login=failed`,
		}),
		(req: any, res) => {
			login(res, req.user);
			res.redirect(`${env.webUrl}${env.webAppPath}/onboard`);
		},
	);
}
...
export function publicUser(u: any) {
	const isAdmin =
		u.role === "admin" ||
		(!!u.email && env.adminEmails.includes(String(u.email).toLowerCase()));
	return {
		id: String(u._id),
		name: u.name || null,
		email: u.email || null,
