...
export const env = {
  ...
  mongoUri: process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/banglabracket',
  ...
  facebook: {
    ...
    get enabled() { return this.enabledFlag && !!(this.id && this.secret); },
  },

  resend: {
    key: process.env.RESEND_API_KEY || '',
    from: process.env.MAIL_FROM || 'BanglaBracket <login@banglabracket.com>',
...
export function validateEnv(): string[] {
  const warn: string[] = [];
  if (!/^[0-9a-fA-F]{64}$/.test(env.jwtSecret)) warn.push('JWT_SECRET must be 64 hex chars (openssl rand -hex 32). Sessions are insecure without it.');
  if (!/^[0-9a-fA-F]{64}$/.test(env.encKey)) warn.push('ENCRYPTION_KEY must be 64 hex chars (openssl rand -hex 32). ID encryption is disabled without it.');
  if (!env.adminEmails.length) warn.push('ADMIN_EMAILS not set — no one can access the admin panel.');
  if (!env.google.enabled) warn.push('Google login disabled (GOOGLE_CLIENT_ID/SECRET).');
  if (!env.facebook.enabled) warn.push('Facebook login disabled (set FACEBOOK_ENABLED=true once your app is approved).');
  if (!env.resend.enabled) warn.push('Resend not configured — email login codes will print to the server console.');
  return warn;
}
